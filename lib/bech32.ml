  ...
  let ret, bits, acc =
    String.fold_left
      (fun (ret, bits, acc) v ->
         ...
         then invalid_arg ("convertbits_exn: invalid data " ^ string_of_int v);
...
let encode5 ~hrp data =
  ...
  let chk, i =
    String.fold_left
      (fun (a, i) c ->
         Bytes.set buf i c;
         Int32.logxor (polymod_step a) (Char.to_int c land 0x1f |> Int32.of_int), succ i)
      (chk, 0)
      hrp
  in
  Bytes.set buf i '1';
  let chk, i =
    String.fold_left
      ...
      data
  in
  let chk = List.fold_left (fun a () -> polymod_step a) chk [ (); (); (); (); (); () ] in
...
let decode bech32 =
  ...
  | Some (hrp, payload) ->
    ...
    (try
       ...
       |> R.ok
     with
     | Exit -> Error ("invalid hrp char in " ^ hrp))
    >>= fun chk ->
    let chk = polymod_step chk in
    let chk =
      String.fold_left
    ...
    >>= fun chk ->
    if !have_upper <> !have_lower && chk = 1l
    then Ok (hrp, Bytes.unsafe_to_string buf)
